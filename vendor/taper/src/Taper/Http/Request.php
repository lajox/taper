...
use Taper\Interfaces\Http\HeadersInterface;
...
class Request extends Message implements ServerRequestInterface
{
     ...
     *
     * @var \Taper\Collection\Collection
     */
    ...
    public function init(array $data = []) {
        ...
        if ($this->data->base != '/' && strlen($this->data->base) > 0 && strpos($this->data->url, $this->data->base) === 0) {
            $this->data->url = substr($this->data->url, strlen($this->data->base));
        }
        ...
        // Merge URL query parameters with $_GET
            ...
            $_GET += self::parseQuery($this->data->url);

            $this->data->query->setData($_GET);
        }

        // Check for JSON input
        if (strpos($this->data->type, 'application/json') === 0) {
            $body = $this->getBody();
            if ($body != '') {
    ...
    public function getMethod()
    {
        if ($this->method === null) {
            ...
            if (empty($this->method)) {
                $this->method = self::getVar('REQUEST_METHOD', 'GET');
